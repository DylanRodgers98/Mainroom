...
router.get('/', (req, res, next) => {
    ...
    User.paginate(query, options, (err, result) => {
        if (err) {
            LOGGER.error('An error occurred when finding users: {}', `${err.toString()}\n${err.stack}`);
            next(err);
        } else {
            res.json({
...
router.patch('/:username', loginChecker.ensureLoggedIn(), isAuthorised, (req, res, next) => {
    ...
    User.findOneAndUpdate({username}, updateQuery, {new: true},(err, user) => {
        if (err) {
            LOGGER.error('An error occurred when updating user {}: {}', username, `${err.toString()}\n${err.stack}`);
            next(err);
        } else if (!user) {
            res.status(404).send(`User (username: ${escape(username)}) not found`);
        } else {
...
router.post('/:username/chat-colour', loginChecker.ensureLoggedIn(), isAuthorised, (req, res, next) => {
    ...
    User.findOneAndUpdate({
        ...
        if (err) {
            LOGGER.error(`An error occurred when updating user {}'s chat colour: {}`, username, `${err.toString()}\n${err.stack}`);
            next(err);
        } else if (!user) {
            res.status(404).send(`User (username: ${escape(username)}) not found`);
        } else {
...
router.put('/:userId/profile-pic', loginChecker.ensureLoggedIn(), isAuthorised, (req, res, next) => {
    ...
    if (userId) {
        s3UploadProfilePic(req, res, async err => {
            if (err) {
                LOGGER.error('An error occurred when uploading profile pic to S3 for user {}: {}', userId, `${err.toString()}\n${err.stack}`);
                return next(err);
            }
            try {
                ...
                LOGGER.error('An error occurred when updating profile pic info for user (_id: {}): {}', userId, `${err.toString()}\n${err.stack}`);
                next(err);
            }
        });
    }
...
const getSubscribersOrSubscriptions = key => (req, res, next) => {
    ...
    User.aggregate([
        ...
        if (err) {
            LOGGER.error('An error occurred when counting number of {} for user {}: {}', key, username, `${err.toString()}\n${err.stack}`);
            next(err);
        } else {
            ...
            User.findOne({username}, key)
                ...
                .exec((err, user) => {
                    if (err) {
                        LOGGER.error('An error occurred when getting {} for user {}: {}', key, username, `${err.toString()}\n${err.stack}`);
                        next(err);
                    } else if (!user) {
                        res.status(404).send(`User (username: ${escape(username)}) not found`);
                    } else {
...
router.get('/:username/subscribed-to/:otherUsername', (req, res, next) => {
    ...
    User.findOne({username: otherUsername}, 'subscribers', (err, otherUser) => {
        if (err) {
            LOGGER.error('An error occurred when finding user {}: {}', otherUsername, `${err.toString()}\n${err.stack}`);
            next(err);
        } else if (!otherUser) {
    ...
    User.findOne({username: username}, (err, user) => {
        if (err) {
            ...
            User.findOne({username: usernameToSubscribeTo}, 'subscribers',(err, userToSubscribeTo) => {
                if (err) {
                    LOGGER.error('An error occurred when finding user {}: {}', usernameToSubscribeTo, `${err.toString()}\n${err.stack}`);
                    next(err);
                        ...
                        userToSubscribeTo.updateOne({$push: {subscribers: {user: user._id}}}, err => {
                            if (err) {
                                LOGGER.error(`An error occurred when adding user {} to user {}'s subscribers: {}`,
                                    username, usernameToSubscribeTo, `${err.toString()}\n${err.stack}`);
                                next(err);
                            } else {
                                user.updateOne({$push: {subscriptions: {user: userToSubscribeTo._id}}}, err => {
                                    if (err) {
                                        LOGGER.error(`An error occurred when adding user {} to user {}'s subscriptions: {}`,
                                            usernameToSubscribeTo, username, `${err.toString()}\n${err.stack}`);
                                        next(err);
                                    } else {
                                        res.sendStatus(200);
                                    }
    ...
    User.findOne({username}, (err, user) => {
        if (err) {
            ...
            User.findOne({username: usernameToUnsubscribeFrom}, 'subscribers', (err, userToUnsubscribeFrom) => {
                if (err) {
                    LOGGER.error('An error occurred when finding user {}: {}', usernameToUnsubscribeFrom, `${err.toString()}\n${err.stack}`);
                    next(err);
                    ...
                    if (isSubscribed) {
                        userToUnsubscribeFrom.updateOne({$pull: {subscribers: {user: user._id}}}, err => {
                            if (err) {
                                LOGGER.error(`An error occurred when removing user {} to user {}'s subscribers: {}`, username, userToUnsubscribeFrom, `${err.toString()}\n${err.stack}`);
                                next(err);
                            } else {
                                user.updateOne({$pull: {subscriptions: {user: userToUnsubscribeFrom._id}}}, err => {
                                    if (err) {
                                        LOGGER.error(`An error occurred when removing user {} to user {}'s subscriptions: {}`, userToUnsubscribeFrom, username, `${err.toString()}\n${err.stack}`);
                                        next(err);
                                    } else {
                                        res.sendStatus(200);
                                    }
...
router.get('/:username/stream-info', async (req, res, next) => {
    ...
    try {
        ...
        LOGGER.error(`An error occurred when finding user {}'s stream info: {}`, username, `${err.toString()}\n${err.stack}`);
        next(err);
    }
});
...
router.patch('/:username/stream-info', loginChecker.ensureLoggedIn(), isAuthorised, (req, res, next) => {
    ...
    User.findOneAndUpdate({
        ...
        if (err) {
            LOGGER.error(`An error occurred when updating user {}'s stream info: {}`, username, `${err.toString()}\n${err.stack}`);
            next(err);
        } else if (!user) {
            res.status(404).send(`User (username: ${escape(username)}) not found`);
        } else {
...
router.post('/:username/stream-key', loginChecker.ensureLoggedIn(), isAuthorised, (req, res, next) => {
    ...
    User.findOneAndUpdate({
        ...
        if (err) {
            LOGGER.error(`An error occurred when updating user {}'s stream key: {}`, username, `${err.toString()}\n${err.stack}`);
            next(err);
        } else if (!user) {
            res.status(404).send(`User (username: ${escape(username)}) not found`);
        } else {
...
router.get('/:username/schedule', (req, res, next) => {
    ...
    User.findOne({username: username}, 'subscriptions nonSubscribedScheduledStreams', (err, user) => {
        if (err) {
            LOGGER.error(`An error occurred when getting finding user {}: {}`, username, `${err.toString()}\n${err.stack}`);
            next(err);
        } else if (!user) {
            res.status(404).send(`User (username: ${escape(username)}) not found`);
        } else {
            ...
            .exec((err, scheduledStreams) => {
                if (err) {
                    LOGGER.error(`An error occurred when getting user {}'s schedule: {}`, username, `${err.toString()}\n${err.stack}`);
                    next(err);
                } else {
...
router.get('/:username/schedule/non-subscribed', (req, res, next) => {
    ...
    User.findOne({username})
        ...
        .exec((err, user) => {
            if (err) {
                LOGGER.error(`An error occurred when retrieving non-subscribed scheduled streams for user {}: {}`, username, `${err.toString()}\n${err.stack}`);
                next(err);
            } else if (!user) {
                res.status(404).send(`User (username: ${escape(username)}) not found`);
            } else {
...
router.patch('/:username/schedule/add-non-subscribed/:scheduledStreamId', loginChecker.ensureLoggedIn(), isAuthorised, (req, res, next) => {
    ...
    User.findOneAndUpdate({
        ...
        if (err) {
            LOGGER.error(`An error occurred when adding non-subscribed scheduled stream (ID: {}) to user {}'s schedule: {}`, scheduledStreamId, username, `${err.toString()}\n${err.stack}`);
            next(err);
        } else if (!user) {
            res.status(404).send(`User (username: ${escape(username)}) not found`);
        } else {
...
router.patch('/:username/schedule/remove-non-subscribed/:scheduledStreamId', loginChecker.ensureLoggedIn(), isAuthorised, (req, res, next) => {
    ...
    User.findOneAndUpdate({
        ...
        if (err) {
            LOGGER.error(`An error occurred when removing non-subscribed scheduled stream (ID: {}) to user {}'s schedule: {}`, scheduledStreamId, username, `${err.toString()}\n${err.stack}`);
            next(err);
        } else if (!user) {
            res.status(404).send(`User (username: ${escape(username)}) not found`);
        } else {
...
router.patch('/:userId/settings', loginChecker.ensureLoggedIn(), isAuthorised, (req, res, next) => {
    ...
    if (isUpdatingUsernameOrEmail) {
        User.find(findQuery, 'username email', (err, users) => {
            if (err) {
                LOGGER.error(`An error occurred when finding users with query {}: {}`, JSON.stringify(findQuery), `${err.toString()}\n${err.stack}`);
                next(err);
            } else if (users) {
...
function updateUserSettings(updateQuery, req, res, next) {
    ...
    User.findByIdAndUpdate(userId, updateQuery, (err, user) => {
        if (err) {
            LOGGER.error(`An error occurred when updating user settings for user with _id {}: {}`, userId, `${err.toString()}\n${err.stack}`);
            next(err);
        } else if (!user) {
            res.status(404).send(`User (_id: ${escape(userId)}) not found`);
        } else {
...
router.delete('/:userId', loginChecker.ensureLoggedIn(), isAuthorised, (req, res, next) => {
    ...
    User.findById(userId, (err, user) => {
        if (err) {
            LOGGER.error(`An error occurred when finding user (_id: {}) in database: {}`, userId, `${err.toString()}\n${err.stack}`);
            next(err);
        } else if (!user) {
            res.status(404).send(`User (username: ${escape(userId)}) not found`);
        } else {
            ...
            User.findByIdAndDelete(userId, err => {
                if (err) {
                    LOGGER.error(`An error occurred when deleting user (_id: {}) from database: {}`, userId, `${err.toString()}\n${err.stack}`);
                    next(err);
                } else {
                    res.sendStatus(200);
                }
