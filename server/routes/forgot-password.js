...
router.post('/', loginChecker.ensureLoggedOut(), (req, res, next) => {
    ...
    User.findOne({email}, '_id email username displayName', (err, user) => {
        if (err) {
            LOGGER.error('An error occurred when finding user with email {}: {}', email, err.stack || err.toString());
            next(err);
        } else if (!user) {
            ...
            randomBytes(16, (err, bytes) => {
                if (err) {
                    LOGGER.error('An error occurred when generating random bytes for password reset token: {}', err.stack || err.toString());
                    next(err);
                } else {
                    ...
                    passwordResetToken.save(err => {
                        if (err) {
                            LOGGER.error('An error occurred when saving password reset token for user with email {}: {}', email, err.stack || err.toString());
                            next(err);
                        } else {
...
router.get('/reset', loginChecker.ensureLoggedOut(), (req, res, next) => {
    ...
    PasswordResetToken.findOne({tokenHash}, (err, token) => {
        if (err) {
            LOGGER.error('An error occurred when finding password reset token: {}', err.stack || err.toString());
            next(err);
        } else if (!token) {
...
router.post('/reset', loginChecker.ensureLoggedOut(), (req, res, next) => {
    ...
    User.findById(userId).exec((err, user) => {
        if (err) {
            ...
            LOGGER.error(`Could not find user (_id: {}) when resetting password: {}`, userId, err.stack || err.toString());
                ...
                if (err) {
                    ...
                    PasswordResetToken.findByIdAndDelete(sanitisedQuery.passwordResetToken._id, err => {
                        LOGGER.error('An error occurred when trying to delete PasswordResetToken (_id: {}) from database. ' +
                            ...
                            sanitisedQuery.passwordResetToken._id, err.stack || err.toString());
                    });
                    res.redirect('/login');
                }
            });
