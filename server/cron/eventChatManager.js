...
const job = new CronJob(cronTime.eventChatManager, async () => {
    ...
    if (rejectedPromises.length) {
        ...
        LOGGER.error('Errors occurred when opening/closing/alerting event chats: {}', err.stack);
...
async function openChats(thisTimeTriggered) {
    ...
    try {
        ...
        LOGGER.error('An error occurred when getting events whose chats need to be opened: {}', err.stack);
        throw err;
    }
...
async function closeChats(thisTimeTriggered) {
    ...
    try {
        ...
        LOGGER.error('An error occurred when getting events whose chats need to be closed: {}', err.stack);
        throw err;
    }
...
async function sendClosureAlerts(thisTimeTriggered) {
    ...
    if (rejectedPromises.length) {
        const err = new CompositeError(rejectedPromises.map(promise => promise.reason));
        LOGGER.error('Errors occurred when sending alerts to event chats about imminent closure: {}', err.stack);
        throw err;
    }
}
